...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (!email || !password || !username || !firstName || !lastName) {
      return alert("REQUIRED: ALL_FIELDS_MUST_BE_FILLED");
    }
    ...
    try {
      ...
      console.error("DEBUG_FULL_ERROR:", error.toJSON());
      const serverMessage = error.response?.data?.message || "NETWORK_ERROR";
      console.error("🔴 REG_ERROR:", serverMessage);
      alert(
        `REGISTRATION_FAILED: ${Array.isArray(serverMessage) ? serverMessage[0] : serverMessage}`,
      );
    } finally {
      setIsLoading(false);
    }
      ...
      >
        <View style={styles.background}>
          ...
          <View style={styles.glassCard}>
            ...
            <Text style={styles.label}>ENCRYPTION_KEY (PASSWORD)</Text>
            <TextInput
              style={styles.input}
              placeholder="••••••••"
              placeholderTextColor="#444"
